"""
22_visem_augment_v4.py
─────────────────────────────────
Figure 2 v4: 1단 배치 최적화
...
def main():
    print("=" * 80)
    print("  Figure 2 v4: 1단 배치 최적화 (3×2)")
    print("=" * 80)
    ...
    # ── Figure 그리기: 3행 2열 (세로 긴 비율) ─────────────
    # figsize: 1단 너비에 맞게 세로 길게
    # 6:8 비율 → 1단 표시 시 글자 보임
    ...
    plt.subplots_adjust(
        left=0.06,
        right=0.94,
        top=0.93,    # Source Domain 라벨 공간
        bottom=0.06,
        wspace=0.08,
        hspace=0.30  # 행 간 간격 충분히
    )
    ...
    # 도메인 라벨 - 큰 글자, 명확한 위치
    fig.text(0.5, 0.965, 'Source Domain (MHSMA)',
    ...
    output_path = (PROJECT_ROOT + r'\paper\figures'
                   r'\fig2_v4_visem_augment.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight',
                 facecolor='white')
    plt.close()
    
    print(f"\n📁 저장: {output_path}")
    print("\n📐 비율: 7:9.5 (3행 2열, 세로 긴 형태)")
    print("📐 1단 너비 가정: 약 82mm")
    print("📐 글자 크기: 15pt → 1단 표시 시 약 8pt (잘 보임)")
    print("=" * 80)
    print("  ✅ Figure 2 v4 생성 완료")
    print("=" * 80)
